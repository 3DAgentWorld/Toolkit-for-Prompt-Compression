...
class LongLLMLinguaCompressor(AbstractCompressor):
    ...
    def load_model(
        ...
        config = AutoConfig.from_pretrained("/home/hdd/lijinyi/CompressionInAvalon/src/models/meta-llama/Llama-2-7b-chat-hf")
        tokenizer = AutoTokenizer.from_pretrained("/home/hdd/lijinyi/CompressionInAvalon/src/models/meta-llama/Llama-2-7b-chat-hf")
...
if __name__ == '__main__':
    ...
    compressor = LongLLMLinguaCompressor(device_map="cpu")
    ...
    start = time.time()
    # match = re.search("(?<=<<SYS>>\n).*?(?=<</SYS>>)", test_prompt, re.S)
    # system_prompt = match.group() if match else ""
    result = compressor.compress(context=test_prompt, ratio=0.3)
